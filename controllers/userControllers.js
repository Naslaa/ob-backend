...
router.post('/login',async(req, res) => {
   ...
   router.post("/forget_password", async(req, res)=>{
    ...
    const transporter= nodemailer.createTransport({
     service:"gmail",
     auth:{
        user:'peakpackbag@gmail.com',
        pass:'jyivjqfkzyijmfbn'
     }   
    });

var mailOptions={
    from:'peakpackbag@gmail.com',
    to : email,
...
transporter.sendMail(mailOptions, (error, info)=>{
    ...
    res.json("verification email sent")
        ...
        res.status(500).json({msg:"verification email not sent"});
    }
});
...
{
res.status(500).json({msg:"password reset link not valid or expired"})
}
